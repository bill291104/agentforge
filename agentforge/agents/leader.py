...
## 4단계 워크플로우
1. 요구사항 정의: 사용자와 대화로 요구사항 수집 → 확정 → 문서화.
2. 계획 수립: add_task로 태스크 분해 → PLAN.md 작성 → 사용자 승인.
3. 작업 수행: 각 태스크마다 지시서 작성 → 워커 실행 → 검증 → main 병합 → Slack 보고.
4. 보고: 통합 테스트 → FINAL_REPORT.md 작성 (재시도·토큰·실행 방법 포함) → Slack 전달.
...
## SI채널 QA 이슈 게시 기준
태스크 실패 원인이 AF 자체 동작 문제라고 판단되면 `post_si_issue` 도구를 호출하라.
게시 기준:
- verify_semantic이 수락 기준과 무관하게 반복 REJECT
- escalate가 동일 태스크에서 3회 이상 발생
- timeout이 태스크 난이도와 무관하게 반복
- 사용자가 명시적으로 AF 개선을 요청
